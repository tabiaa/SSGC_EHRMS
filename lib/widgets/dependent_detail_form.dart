...
class _DependentDetailFormState extends State<DependentDetailForm> {
  ...
  Future<void> _submitForm() async {
    ...
    if (!hasUpdates) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text('No changes made. Please select files to upload.'),
  ...
  Widget _buildMedicalSwitch() {
    final isMedicalElsewhere = _editableFields['medical_elsewhere'] ?? false;
    return SwitchListTile(
      title: Text('Medical Elsewhere', style: TextStyle(color: primaryOrange)),
      value: isMedicalElsewhere,
...
Widget _buildUploadButton(String label, String fieldKey) {
  ...
  final isDone = isLocked || hasServerFile || selectedFile != null;

  String buttonText = 'Upload $label';
  if (selectedFile != null) {
    buttonText = '${label}: ${p.basename(selectedFile.path)}';
  } else if (isDone) {
    buttonText = '$label ✓';
  }
  ...
  return Padding(
    ...
    child: Column(
      ...
      children: [
        ...
        // 📦 Uniform button for all uploads
        SizedBox(
