...
class _LoginScreenState extends State<LoginScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _login() async {
    ...
    if (id == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Invalid Employee ID'), backgroundColor: Colors.redAccent),
      );
      return;
    }
    ...
    try {
      ...
      if (e.response?.statusCode == 401) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Wrong credentials. Please try again.'), backgroundColor: Colors.redAccent),
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Network error: ${e.message}'), backgroundColor: Colors.redAccent),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _launchHelpUrl() async {
    final Uri url = Uri.parse('https://viewbill.ssgc.com.pk/EHRMS_APP_GUIDE');
    if (!await launchUrl(url, mode: LaunchMode.externalApplication)) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Could not launch help link'), backgroundColor: Colors.redAccent),
      );
    }
  }
              ...
              child: Column(
                ...
                children: [
                  ...
                  const Text(
                    ...
                    style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w700, letterSpacing: 1.2),
                  ),
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            TextButton(
                              ...
                              child: const Text('Help?', style: TextStyle(decoration: TextDecoration.underline)),
                            ),
                          ],
                        ),
                      ),
                  ...
                  Row(
                    ...
                    children: [
                      const Text("Don't have an account? ", style: TextStyle(color: Colors.white, fontSize: 14)),
                      GestureDetector(
