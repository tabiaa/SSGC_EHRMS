...
class _ForgotPasswordScreenState extends State<ForgotPasswordScreen> {
  ...
  Future<void> _requestOtp() async {
    ...
    try {
      final res = await ApiService.requestForgotPasswordOtp(empId);
      setState(() {
        message = res['message'] ?? '';
  ...
  Future<void> _verifyOtp() async {
    ...
    setState(() => isLoading = true);
    try {
      final res = await ApiService.verifyForgotPasswordOtp(employeeId: empId, otp: otp);
      setState(() {
        message = res['message'] ?? '';
  ...
  Future<void> _resetPassword() async {
    ...
    final passwordRegex = RegExp(r'^(?=.*[A-Za-z])(?=.*\d)(?=.*[^A-Za-z0-9]).{8,}$');
    ...
    setState(() => isLoading = true);
    try {
      final res = await ApiService.resetPassword(employeeId: empId, newPassword: password);

      if (res['success'] == true) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("Password reset successfully! Redirecting..."),
        ...
        setState(() => message = res['message'] ?? 'Error resetting password');
      }
    } catch (e) {
    ...
    if (currentStep == 1) {
      stepWidget = Column(
        children: [
          ...
          // const Text(
          //   "Enter your Employee ID to receive an OTP for password reset",
          //   textAlign: TextAlign.center,
                ...
                children: [
                  ...
                  if (message.isNotEmpty)
                    Text(
                      ...
                      textAlign: TextAlign.center,
                      style: TextStyle(
                        color: message.contains("Success") || message.contains("sent")
                            ? Colors.green
                            : Colors.red,
                      ),
                    ),
