...
class _DependentsScreenState extends State<DependentsScreen> {
  ...
  Future<void> _loadDependents() async {
    try {
      // ✅ No employeeId needed — API uses JWT token
  ...
  Widget build(BuildContext context) {
    ...
    final employeeId = employee?.id ?? 'Unknown';
    final fullName = employee?.fullName ?? 'Employee';
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Icon(Icons.family_restroom_outlined, size: 90, color: ssgcOrange.withOpacity(0.4)),
            const SizedBox(height: 20),
            const Text(
              'No dependents found',
              style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFFEA7600)),
            ),
            const SizedBox(height: 6),
            Text(
              'You can add or update dependents from the form section.',
        ...
        child: Container(
          decoration: BoxDecoration(
            gradient: const LinearGradient(
              colors: [Color(0xFFFFFFFF), Color(0xFFFFF4E8)],
              begin: Alignment.topLeft,
              end: Alignment.bottomRight,
            ),
            borderRadius: BorderRadius.circular(16),
            border: Border.all(color: ssgcOrange.withOpacity(0.15)),
          ),
          ...
          child: Row(
            children: [
              CircleAvatar(
                radius: 26,
                backgroundColor: ssgcOrange.withOpacity(0.15),
                ...
                child: dep.profilePictureUrl?.isNotEmpty == true
                    ? null
                    : const Icon(Icons.person, color: ssgcOrange, size: 30),
              ),
              ...
              const Icon(Icons.arrow_forward_ios, size: 16, color: ssgcOrange),
            ],
          ),
        ),
      ),
